...
class LivePhotoCaptureSessionManager: NSObject {
    ...
    private func _configureSession(isRearCamera:Bool) {
        ...
        do {
            guard let videoDevice = AVCaptureDevice.default(
                ...
                for: .video,
                position: isRearCamera==true ? AVCaptureDevice.Position.back : AVCaptureDevice.Position.front) else {fatalError()}
    ...
    func configureSession(isRearCamera:Bool) {
        /*
        ...
        sessionQueue.async { [unowned self] in
            self._configureSession(isRearCamera: isRearCamera)
        }
    }
    ...
    func capture(videoOrientation: AVCaptureVideoOrientation, handler: @escaping ((Int) -> Void)) {
        ...
        sessionQueue.async {
            ...
            let deviceForTourch = AVCaptureDevice.default(for: AVMediaType.video)
            if (deviceForTourch?.hasTorch ?? false) {
