...
class PhotosController: UICollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        //Header View
        let viewHeader = UIView.init(frame: CGRect.init(x: 0, y: 0, width: UIScreen.main.bounds.size.width, height: 64))
        viewHeader.backgroundColor = .white
        self.view.addSubview(viewHeader)
        
        //Back Button
        let controlBack = UIControl.init(frame: CGRect.init(x: 0, y: 20, width: 44, height: 44))
        controlBack.backgroundColor = .clear
        controlBack.addTarget(self, action:#selector(self.tappedOnBack), for: .touchUpInside)
        viewHeader.addSubview(controlBack)
        
        let imgBack = UIImageView.init(frame: CGRect.init(x: 15, y: 15, width: 18, height: 18))
        imgBack.image = UIImage.init(named: "imgBackArrow")
        imgBack.contentMode = .scaleAspectFit
        controlBack.addSubview(imgBack)
    }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        //Colv Scroll To End
        self.colvScrollToEnd()
    }
    func colvScrollToEnd() {
